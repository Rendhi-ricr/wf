        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="text-end">
                    ...
                    <a href="<?= site_url('panel/agenda/tambah')?>" class="btn btn-primary"><i class="ti ti-plus ti-sm"></i> Tambah</a>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <div class="tab-content" id="pills-tabContent">
                        <div class="tab-pane fade show active" id="publish" role="tabpanel" aria-labelledby="publish-tab" tabindex="0">
                            ...
                            <div class="table-responsive">
                                <table id="agenda-publish-ajax" class="table">
                                    <thead>
                                        <tr>
                        ...
                        <div class="tab-pane fade" id="draft" role="tabpanel" aria-labelledby="draft-tab" tabindex="0">
                            ...
                            <div class="table-responsive">
                                <table id="agenda-draft-ajax" class="table">
                                    <thead>
                                        <tr>
                        ...
                        <div class="tab-pane fade" id="trash" role="tabpanel" aria-labelledby="trash-tab" tabindex="0">
                            ...
                            <div class="table-responsive">
                                <table id="agenda-trash-ajax" class="table">
                                    <thead>
                                        <tr>
                                            <th>No</th>
                                            <th>Judul</th>
...
$(document).ready(function() {
    var tablePublish = $('#agenda-publish-ajax').DataTable({
        processing: true,
        serverSide: true,
        ordering: false,
        ajax: {
            url: '<?= site_url("panel/agenda/fetch-publish");?>',
            type: "POST",
            data: function(d) {
    ...
    $('#agenda-publish-ajax tbody').on('click', 'input.row-select', function () {
    ...
    $('#confirm-delete-btn').on('click', function () {
        ...
        if (ids.length > 0) {
            $.ajax({
                url: "<?= site_url('panel/agenda/trash-selected'); ?>",
                method: 'POST',
                data: { ids: ids },
    ...
    // agendaDraft
    var tableDraft = $('#agenda-draft-ajax').DataTable({
        processing: true,
        serverSide: true,
        ordering: false,
        ajax: {
            url: '<?= site_url("panel/agenda/fetch-draft");?>',
            type: "POST",
            data: function(d) {
                // console.log(d); // Tambahkan ini untuk debugging
                return d;
    ...
    // Handle row checkbox click
    $('#agenda-draft-ajax tbody').on('click', 'input.row-select', function () {
        $(this).closest('tr').toggleClass('selected');
    });
    ...
    // agendaTrash
    var tableTrash = $('#agenda-trash-ajax').DataTable({
        processing: true,
        serverSide: true,
        ordering: false,
        ajax: {
            url: '<?= site_url("panel/agenda/fetch-trash");?>',
            type: "POST",
            data: function(d) {
                // console.log(d); // Tambahkan ini untuk debugging
                return d;
    ...
    // Handle row checkbox click
    $('#agenda-trash-ajax tbody').on('click', 'input.row-select', function () {
        $(this).closest('tr').toggleClass('selected');
    });
