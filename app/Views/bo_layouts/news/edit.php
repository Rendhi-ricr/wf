...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <!-- Tambah::Start -->
    <form id="editor-form" action="<?= site_url('panel/news/' . $dataNews['id_news'] . '/edit') ?>" method="post" enctype="multipart/form-data">
        <div class="card">
            <div class="card-body">
                    ...
                    <div class="col-md-8">
                        <div class="row mb-3">
                            <div class="col-md-12">
                                ...
                                <input type="text" name="title_news" value="<?= $dataNews['title_news'] ?>" id="judul" class="form-control mb-3" />
                            </div>
                            <div class="col-md-12">
                                <label class="form-label" for="snow-editor">Description</label>
                                <div id="snow-editor"><?= $dataNews['desc_news'] ?></div>
                                <textarea name="desc_news" id="content" class="d-none"><?= $dataNews['desc_news'] ?></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        ...
                        <div class="form-check">
                            <input type="radio" id="status-publish" name="status" value="publish" class="form-check-input" <?= ($dataNews['status'] === 'publish') ? 'checked' : ''; ?> />
                            <label class="form-check-label badge bg-primary mb-3" for="status-publish">Publish</label>
                        </div>
                        <div class="form-check">
                            <input type="radio" id="status-draft" name="status" value="draft" class="form-check-input" <?= ($dataNews['status'] === 'draft') ? 'checked' : ''; ?> />
                        ...
                        <?php if ($dataNews['photo_cover']) : ?>
                            <img id="preview" src="<?= base_url('uploads/news/' . $dataNews['photo_cover']) ?>" alt="Preview Image" class="img-thumbnail mb-3" style="max-width: 100%; height: auto;">
                        <?php else : ?>
                        ...
                        <div>
                            ...
                            <textarea name="photo_desc" id="pd" class="form-control "><?= $dataNews['photo_desc'] ?></textarea>
                        </div>
