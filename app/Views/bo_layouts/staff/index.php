        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="text-end">
                    ...
                    <a href="<?= site_url('panel/staff/tambah') ?>" class="btn btn-primary"><i class="ti ti-plus ti-sm"></i> Tambah</a>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table id="staff-ajax" class="table">
                            <thead>
                                <tr>
...
<script>
    $(document).ready(function() {
        var table = $('#staff-ajax').DataTable({
            processing: true,
            serverSide: true,
            ordering: false,
            ajax: {
                url: '<?= site_url("panel/staff/fetch"); ?>',
                type: "POST",
                data: function(d) {
                    return d;
                }
            ...
            columnDefs: [{
                "targets": [1], // Indestaff kolom yang akan disembunyikan
                "visible": false // Atur visibilitas kolom menjadi false
            }],
        });

        // Handle row checkbox click
        $('#staff-ajax tbody').on('click', 'input.row-select', function() {
        ...
        $('#confirm-delete-btn').on('click', function() {
            ...
            if (ids.length > 0) {
                $.ajax({
                    url: "<?= site_url('panel/staff/trash-selected'); ?>",
                    method: 'POST',
                    data: {
                        ids: ids
                    },
